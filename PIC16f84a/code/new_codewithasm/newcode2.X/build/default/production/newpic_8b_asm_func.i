...
EECON2 equ 0089h
# 4 "newpic_8b_asm_func.s" 2

              __config _WDT_OFF & _XT_OSC & _PWRTE_OFF
;-----------------------------------------
           goto start
start: bsf STATUS,((STATUS) and 07Fh), 5
         bsf TRISA,0 ;set ((PORTA) and 07Fh), 0 as input
         bcf TRISB,7 ;set ((PORTB) and 07Fh), 7 as output
         bcf STATUS,((STATUS) and 07Fh), 5
         goto MAIN
;-----------------------------------------
MAIN: bcf PORTB,7 ;set a 0 bit at output i.e pin ((PORTB) and 07Fh), 7
loop: btfss PORTA,0 ;test ((PORTA) and 07Fh), 0 and skip is ((PORTA) and 07Fh), 0=1
         goto MAIN ;goto MAIN
         bsf PORTB,7 ;set a 1 bit at ((PORTB) and 07Fh), 7
         goto loop
;---------------------------------------
end
